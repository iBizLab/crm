## 未关闭的任务（关联）(not_close_task_related) <!-- {docsify-ignore-all} -->
...
(`CLOSED_TIME(关闭时间)` ISNULL AND `TYPE(类型)` EQ `'TASK'`)



### 查询连接
* **RELATION存在1:N（EXISTS (SELECT)）DERCUSTOM_TAGET_RELATION_TASK**<br>
