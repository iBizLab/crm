## 我的待办任务(my_todo_task) <!-- {docsify-ignore-all} -->
...
(`TYPE(类型)` EQ `'TASK'` AND (`STATUS(状态)` EQ `'10'` OR `STATUS(状态)` EQ `'30'` OR `STATUS(状态)` EQ `'50'`))



### 查询连接
* **TASK_EXECUTOR存在1:N（EXISTS (SELECT)）DER1N_TASK_EXECUTOR_TASK_TASK_ID**<br>
连接关系：[DER1N_TASK_EXECUTOR_TASK_TASK_ID](der/DER1N_TASK_EXECUTOR_TASK_TASK_ID)<br>
连接实体：[任务&活动](module/crm/task)<br>
连接条件：(`USER_ID(人员标识)` EQ `用户上下文.userid`)<br>
