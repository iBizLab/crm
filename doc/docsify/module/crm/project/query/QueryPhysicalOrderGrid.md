## 查询实体订单(QueryPhysicalOrderGrid) <!-- {docsify-ignore-all} -->
...
(`IS_PHYSICAL_ORDER(对应实体订单)` EQ `'1'` AND `ORDER_DATE(订单日期)` LT `数据上下文.lt` AND `ORDER_DATE(订单日期)` GTANDEQ `数据上下文.gt` AND `CONCAT(YEAR(T1.ORDER_DATE),'-',LPAD(MONTH(T1.ORDER_DATE), 2, '0')) = #{ctx.datacontext.order_date}`)
