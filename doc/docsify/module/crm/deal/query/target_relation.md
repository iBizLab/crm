## 被关联的商机信息(target_relation) <!-- {docsify-ignore-all} -->
...
### 查询连接
* **RELATION存在1:N（EXISTS (SELECT)）DERCUSTOM_DEAL_RELATION**<br>
连接关系：[DERCUSTOM_DEAL_RELATION](der/DERCUSTOM_DEAL_RELATION)<br>
连接实体：[商机](module/crm/deal)<br>
连接条件：(`TARGET_TYPE(关联目标类型)` EQ `数据上下文.principal_type` AND `TARGET_ID(关联目标标识)` EQ `数据上下文.principal_id`)<br>
